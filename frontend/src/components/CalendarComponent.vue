<template>
    <ion-datetime
        ref="datetime"
        id="calendar"
        :class="{ 'datetime-dark': themeIsDark, 'datetime-light': !themeIsDark }"
        presentation="date"
        display-format="DDDD MMMM D, YYYY"
        picker-format="DDDD MMMM D, YYYY"
        placeholder="Select Date"
        size="cover"
        :highlightedDates="highlightedDates"
        v-model="selectedDate"
    ></ion-datetime>
    <ion-button shape="round" @click="handleSelection"> Select </ion-button>
</template>
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const highlightedDates =
                ref<{ date: string; textColor: string; backgroundColor: string }[]>()
                ...
                const observerCallback = (mutationsList: MutationRecord[]) => {
                    for (const mutation of mutationsList) {
                        if (mutation.type === "attributes") {
                            ...
                            const label = calendar?.shadowRoot?.querySelector(
                                ".calendar-month-year ion-item ion-label",
                            )
            ...
            const handleMonthChange = (dateText: string) => {
                ...
                CalendarService.getCalendarCalendarGet(formattedDate).then(
                    (response) => {
...
<style scoped>
    ...
    .ios .datetime-light {
        --wheel-fade-background-rgb: rgba(var(--ion-color-dark-rgb), 1);
    }
