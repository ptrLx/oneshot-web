<template>
    <ion-page>
        ...
        <ion-content :fullscreen="true">
            ...
            <row-component
                row-height="360px"
                sectionHeaderTitle="Gallery"
                :button-func="() => router.push('/gallery')"
            >
                <swiper-slide v-if="hasImage('random_happy')">
                    <router-link :to="`/image/${flashbackImgs['random_happy']?.meta.date}`">
                        <div class="flashback-container">
                            <ion-img
                                :src="flashbackImgs['random_happy']?.url"
                                alt="Image of a random happy day."
                                class="rounded-image"
                                fetchpriority="high"
                            >
                            </ion-img>
                ...
                <swiper-slide v-else>
                    <card
                        card-title="Your Flashbacks"
                        card-subtitle="Nothing here yet - Upload more images to see them appear here!"
                    >
                        <ion-img
                            src="/icons/512.png"
                            alt="OneShot logo"
                ...
                <swiper-slide v-if="hasImage('last_very_happy_day')">
                    <router-link :to="`/image/${flashbackImgs['last_very_happy_day']?.meta.date}`">
                        <div class="flashback-container">
                            <ion-img
                                :src="flashbackImgs['last_very_happy_day']?.url"
                                alt="Image of the last very happy day."
                                class="rounded-image"
                            >
                            </ion-img>
                ...
                <swiper-slide v-if="hasImage('same_day_last_month')">
                    <router-link :to="`/image/${flashbackImgs['same_day_last_month']?.meta.date}`">
                        <div class="flashback-container">
                            <ion-img
                                :src="flashbackImgs['same_day_last_month']?.url"
                                alt="Image of the same day last month."
                                class="rounded-image"
                            >
                            </ion-img>
                ...
                <swiper-slide
                    v-for="(image, index) in getSameDateLastYearsImages(flashbackImgs)"
                    :key="index"
                >
                    <router-link :to="`/image/${image.meta.date}`">
                        <div class="flashback-container">
                            <ion-img
                                :src="image.url"
                                :alt="`Image of the day ${index} years ago.`"
                                class="rounded-image"
                            >
                            </ion-img>
            ...
            >
                <swiper-slide>
                    <card>
                        <donut-chart
                            center-text="Happiness Week"
                            :data="stats?.happiness_current_week"
                        ></donut-chart>
                    </card>
                </swiper-slide>
                <swiper-slide>
                    <card>
                        <donut-chart
                            center-text="Happiness Month"
                            :data="stats?.happiness_current_month"
                        ></donut-chart>
                    </card>
                </swiper-slide>
                <swiper-slide>
                    <card>
                        <donut-chart
                            center-text="Happiness Year"
                            :data="stats?.happiness_current_year"
                        ></donut-chart>
                    </card>
                </swiper-slide>
            </row-component>
...
<script setup lang="ts">
    ...
    const updateActions = (
        event: CustomEvent = { detail: { complete: () => {} } } as CustomEvent,
    ) => {
        getFlashbacks()
            .then((flashbacks) => {
        ...
        StatisticsService.getStatisticsStatsGet()
            .then((response) => {
