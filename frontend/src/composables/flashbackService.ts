...
export const useFlashbackService = () => {
    ...
    const getFlashbacks = async (): Promise<{ [key: string]: FlashbackUrlAndMeta }> => {
        ...
        await OneShotService.getFlashbacksFlashbackGet().then(
            async (res) => {
                ...
                for (const key of Object.keys(flashbacks)) {
                    ...
                    if (Array.isArray(flashback)) {
                        ...
                        for (const item of flashback) {
                            await downloadGalleryImg(item.date, isPreview).then(
                                (blob) => {
                        ...
                        await downloadGalleryImg(flashback?.date ?? "", isPreview).then(
                            (blob) => {
                                const img = URL.createObjectURL(blob)
                                flashbackImgs[key] = {
                                    url: img,
